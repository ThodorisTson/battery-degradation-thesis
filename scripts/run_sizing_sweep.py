...
from degradation.subgradient import fit_shi_polynomial
...
def plot_npv_three_models(results: List[dict], params: dict, n: int, ts: str,
    ...
    plt.tight_layout()
    out = OUTPUT_DIR / f"sizing_npv_comparison_{n}h_{ts}.png"
...
def plot_sweep_slices(results: List[dict], params: dict, n: int, ts: str) -> None:
    ...
    plt.tight_layout()
    fig.savefig(OUTPUT_DIR / f"sizing_slices_{ts}.png", dpi=300, facecolor=BG)
    plt.close()
...
def plot_sweep_2d(results: List[dict], params: dict, n: int, ts: str) -> None:
    ...
    fig.suptitle(
        f"Battery degradation and cycling by energy and power capacity ({n} hours)",
    ...
    plt.tight_layout()
    fig.savefig(OUTPUT_DIR / f"sizing_sweep_{ts}.png", dpi=300, facecolor=BG)
    plt.close()
...
def plot_npv_three_models_lifetime(
    ...
    out = OUTPUT_DIR / f"sizing_lifetime_npv_{n}h_{N_YEARS}yr_{ts}.png"
    fig.savefig(out, dpi=150, bbox_inches="tight", facecolor=BG)
